...
class SteamCommunityMobileConfirmations {
    constructor(options) {
        ...
        this.deviceId = options.deviceId || SteamTotp.getDeviceID(this.steamId);
        this.offset = options.steamOffset || 0;
        ...
        if (options.hasOwnProperty('requestOptions')) {
            this._request = this._request.defaults(options.requestOptions);
        }

        // Set each cookie NOM NOM
        for (let cookie of options.webCookie) {
